...
class Consumer(db.Model, UserMixin):
    ...
    def __repr__(self):
        return f"Consumer('{self.fullname}','{self.username}','{self.email}','{self.image_file}', '{self.wallet})"
...
class Pass(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('consumer.id'), nullable=False)

    def __repr__(self):
        return f"Consumer Pass('{self.source}', '{self.dest}', '{self.date}', '{self.user_id}', '{self.id}')"
